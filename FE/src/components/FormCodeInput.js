...
const FormCodeInput = ({ code, setCode }) => {
  ...
  const { setReviewResult, historyItems, setHistoryItems, isLoadingHistory, setIsLoadingHistory } = useCode();
  ...
  useEffect(() => {
    ...
    fetchHistory();
  }, [historyItems.length, isLoadingHistory, setHistoryItems, setIsLoadingHistory]);
              ...
              historyItems.map((item) => (
                ...
                >
                  <div className="text-sm font-medium">
                    {item.language ? `Review ${item.language.toUpperCase()}` : 'Review Code'}
                  </div>
